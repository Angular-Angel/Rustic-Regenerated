...
import static net.angle.rustic.common.blocks.AppleSeedsBlock.AGE;
...
public interface AppleGrowthExporter {
    ...
    public default boolean isImmatureSeeds(BlockState state) {
        return state.is(Rustic.APPLE_SEEDS_BLOCK.get()) && state.getValue(AGE) == 0;
    }
    
    public default boolean isMatureSeeds(BlockState state) {
        return state.is(Rustic.APPLE_SEEDS_BLOCK.get()) && state.getValue(AGE) == 1;
    }
